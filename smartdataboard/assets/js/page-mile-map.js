/* =====================================================================
   광명 스마트데이터보드 · 4종 마일별 데이터 지도  (Naver Maps v3)
   ===================================================================== */
(function () {
  'use strict';

  /* ── 광명시 경계 좌표 (근사 폴리곤, [lat, lng]) ── */
  var BOUNDARY_COORDS = [
  ...
  /* ── 마일 아이콘 SVG (인라인 — Naver DivIcon은 lucide auto-process 불가) ── */
  var MILE_ICONS_SVG = {
  ...
  var CHART_DATA = {
    ...
    data: {
      line:  { labels:['1월','2월','3월','4월','5월'], values:[16000,19000,22000,24800,27480], color:'#6E74D6', unit:'건' },
  ...
  ================================================================ */
  function initMap() {
    map = new naver.maps.Map('gisMap', {
      center: new naver.maps.LatLng(37.454, 126.878),
      zoom: 13,
      zoomControl: false,
    ...
    var paths = BOUNDARY_COORDS.map(function (c) {
      return new naver.maps.LatLng(c[0], c[1]);
    });
    boundaryPolygon = new naver.maps.Polygon({
      map: map,
      paths: [paths],
      strokeColor: '#044E9E',
      strokeWeight: 2.5,
  ...
  function addMarker(pin) {
    ...
    markerRefs.push({ pin: pin, nMarker: marker });
  }
  ...
  function applyMarkerFilter() {
    markerRefs.forEach(function (ref) {
      var p = ref.pin;
      var show = filterMileLayer[p.mile] && filterStatus[p.status];
      ref.nMarker.setMap(show ? map : null);
    });
  }
  ...
  function initSearch() {
    ...
    function renderDropdown(results) {
      ...
      var html = results.slice(0, 8).map(function (p) {
      ...
      dropdown.querySelectorAll('.search-item').forEach(function (item) {
        item.addEventListener('click', function () {
          ...
          closeDropdown();
          map.setCenter(new naver.maps.LatLng(found.lat, found.lng));
          map.setZoom(17, true);
  ...
  function initMapControls() {
    ...
    document.getElementById('btnFitAll').addEventListener('click', function () {
      map.setCenter(new naver.maps.LatLng(37.454, 126.878));
      map.setZoom(13, true);
    });
  ...
  function initTabs() {
    document.querySelectorAll('.mile-tab').forEach(function (tab) {
      tab.addEventListener('click', function () {
        setActiveMile(this.dataset.mile);
      });
    });
  }
  ...
  function setActiveMile(mile) {
    ...
    var lats = milePins.map(function (p) { return p.lat; });
    var lngs = milePins.map(function (p) { return p.lng; });
    var sw = new naver.maps.LatLng(
      Math.min.apply(null, lats) - 0.005,
      Math.min.apply(null, lngs) - 0.005
    );
    var ne = new naver.maps.LatLng(
      Math.max.apply(null, lats) + 0.005,
      Math.max.apply(null, lngs) + 0.005
    );
    map.fitBounds(new naver.maps.LatLngBounds(sw, ne), { top:30, right:30, bottom:30, left:30 });
  }
  ...
  function drawLineChart(mile, data) {
    var canvas = document.getElementById(mile + 'LineChart');
    if (!canvas) return;
    var s = setupCanvas(canvas);
    var ctx = s.ctx, W = s.w, H = s.h;
    var pad = { t:16, r:12, b:8, l:44 };
    var aw = W - pad.l - pad.r;
    var ah = H - pad.t - pad.b;
    var mx = Math.max.apply(null, data.values) * 1.15;
    ...
    [0, 0.25, 0.5, 0.75, 1].forEach(function (t) {
      ...
      var v = Math.round(mx * t);
      ctx.fillText(v >= 1000 ? (v/1000).toFixed(1)+'k' : v, pad.l - 5, y + 4);
    });

    var pts = data.values.map(function (v, i) {
      return { x: pad.l + (i/(n-1))*aw, y: pad.t + ah*(1 - v/mx) };
    });
    ...
    for (var i=1; i<pts.length; i++) {
      var cp = {x:(pts[i-1].x+pts[i].x)/2, y:(pts[i-1].y+pts[i].y)/2};
    ...
    for (var j=1; j<pts.length; j++) {
      var cp2 = {x:(pts[j-1].x+pts[j].x)/2, y:(pts[j-1].y+pts[j].y)/2};
  ...
  function drawBarChart(mile, data) {
    var canvas = document.getElementById(mile + 'BarChart');
    if (!canvas) return;
    var s = setupCanvas(canvas);
    var ctx = s.ctx, W = s.w, H = s.h;
    var pad = { t:10, r:12, b:30, l:44 };
    var aw = W - pad.l - pad.r;
    var ah = H - pad.t - pad.b;
    var mx = Math.max.apply(null, data.values) * 1.2;
    var n  = data.labels.length;
    var gap  = aw / n;
    var barW = Math.floor(gap * 0.55);
