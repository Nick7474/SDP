...
(function () {
  ...
  function drawLineChart(metricKey, hoverIdx) {
    ...
    /* X 라벨 — 데이터 포인트 x 좌표와 1:1 정렬 */
    var xlabels = document.getElementById('chartXLabels');
    if (xlabels) {
      var n = CHART_LABELS.length;
      xlabels.innerHTML = CHART_LABELS.map(function (l, i) {
        var x = padL + (i / (n - 1)) * cw;
        return '<span style="left:' + x + 'px">' + l + '</span>';
      }).join('');
    }
  }
  ...
  function renderTimeline() {
    var tl = document.getElementById('alertTimeline');
    if (!tl) return;
    tl.innerHTML = '<div class="st-timeline__track"></div>';
    var minutesInDay = 24 * 60;
    /* 시각 눈금 (6시간 단위) */
    [0, 6, 12, 18, 24].forEach(function (h) {
      var pct = (h * 60 / minutesInDay) * 100;
      var tick = document.createElement('div');
      tick.className = 'st-timeline__tick';
      tick.style.left = 'calc(12px + (100% - 24px) * ' + (pct/100) + ')';
      tl.appendChild(tick);
      var lbl = document.createElement('div');
      lbl.className = 'st-timeline__lbl';
      lbl.textContent = String(h).padStart(2,'0') + ':00';
      lbl.style.left = 'calc(12px + (100% - 24px) * ' + (pct/100) + ')';
      tl.appendChild(lbl);
    });
    /* 이벤트 마커 */
    ALERTS.forEach(function (a) {
      ...
      var pct   = mins / minutesInDay;
      var dot   = document.createElement('div');
      dot.className = 'st-timeline__dot st-timeline__dot--' + a.type;
      dot.style.left = 'calc(12px + (100% - 24px) * ' + pct + ')';
      dot.title      = a.time + ' — ' + a.content;
      dot.style.top  = '50%';
      dot.addEventListener('click', function () { openDrawer(a); });
      tl.appendChild(dot);
    });
  }
  ...
  function renderAlerts(page) {
    ...
    if (groupMode) {
      ...
      TYPE_ORDER.forEach(function (t) {
        var group = ALERTS.filter(function (a) { return a.type === t; });
        ...
        group.forEach(function (a) { body.appendChild(makeAlertRow(a)); });
      });
      lucide.createIcons();
      return;
    }
  ...
  var dwCloseFooter = document.getElementById('dwCloseFooter');
  if (dwCloseFooter) dwCloseFooter.addEventListener('click', closeDrawer);
  ...
  if (groupToggleBtn) {
    groupToggleBtn.addEventListener('click', function () {
      groupMode = !groupMode;
      groupToggleBtn.classList.toggle('on', groupMode);
      renderAlerts(currentPage);
    });
  }
